...
struct piece *create_user_pieces() {
  ...
  for (int i = 0; i < 8; i++) {
    if (i == 4) {
      user_pieces[i] =
          create_piece(KING, ((i + 1) * 6) + 2 * i, 30, i + 1, 1, WHITE);
      continue;
    }
    user_pieces[i] =
        create_piece(KNIGHT, ((i + 1) * 6) + 2 * i, 30, i + 1, 1, WHITE);
  }
  return user_pieces;
}
...
struct piece *create_computer_pieces() {
  ...
  for (int i = 0; i < 8; i++) {
    if (i == 4) {
      computer_pieces[i] =
          create_piece(KING, ((i + 1) * 6) + 2 * i, 2, i + 1, 8, BLACK);
      continue;
    }
    computer_pieces[i] =
        create_piece(KNIGHT, ((i + 1) * 6) + 2 * i, 2, i + 1, 8, BLACK);
  }
